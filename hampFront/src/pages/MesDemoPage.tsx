...
function createRows(title: string): DemoRow[] {
  return [
    { id: 'MES-001', name: `${title} A`, category: '운영', status: '정상', owner: '관리자', date: '2026-06-18', tone: 'good' },
    { id: 'MES-002', name: `${title} B`, category: '검토', status: '주의', owner: '운영자', date: '2026-06-17', tone: 'warn' },
    { id: 'MES-003', name: `${title} C`, category: '이력', status: '완료', owner: '담당자', date: '2026-06-16', tone: 'info' },
  ]
}
    ...
    <section className="screenStack">
      ...
      <div className="searchBand">
        {['검색어', '구분', '상태', '기간'].map((filter) => (
          <label key={filter}>
            {filter}
            <input value={filter === '검색어' ? keyword : undefined} onChange={(event) => filter === '검색어' && setKeyword(event.target.value)} placeholder={`${filter} 입력`} />
          </label>
        ))}
        <button type="button" className="primaryButton" onClick={() => handleMock('mock 데이터 기준으로 조회되었습니다.')}>
          검색
        </button>
      </div>
      ...
      <Panel title={`${title} 목록`} action="초기화" onAction={() => setKeyword('')}>
        <div className="toolbarRow">
          <button type="button" className="primaryButton" onClick={isSend ? () => handleMock('실제 SMS 발송 없이 발송 완료 처리되었습니다.') : handleAdd}>
            {isSend ? '발송' : '등록'}
          </button>
        </div>
        ...
        <DataTable
          ...
          rows={filteredRows.map((row) => {
            ...
            return [
              <input key={`${row.id}-select`} type="radio" checked={selectedId === row.id} onChange={() => setSelectedId(row.id)} aria-label={`${row.name || row.id} 선택`} />,
              row.id,
              isEditing ? <input className="tableInput" value={row.name} onChange={(event) => updateRow(row.id, 'name', event.target.value)} placeholder="명칭 입력" /> : row.name,
              isEditing ? <input className="tableInput" value={row.category} onChange={(event) => updateRow(row.id, 'category', event.target.value)} placeholder="구분 입력" /> : row.category,
              isEditing ? <input className="tableInput" value={row.status} onChange={(event) => updateRow(row.id, 'status', event.target.value)} placeholder="상태 입력" /> : <Badge tone={row.tone}>{row.status}</Badge>,
              isEditing ? <input className="tableInput" value={row.owner} onChange={(event) => updateRow(row.id, 'owner', event.target.value)} placeholder="담당자 입력" /> : row.owner,
              isEditing ? <input className="tableInput" value={row.date} onChange={(event) => updateRow(row.id, 'date', event.target.value)} placeholder="YYYY-MM-DD" /> : row.date,
              <div className="rowActions">
                {isEditing ? (
                  ...
                  <>
                    <button type="button" className="miniButton" onClick={() => handleEdit(row)}>
                      수정
                    </button>
                    <button type="button" className="miniButton danger" onClick={() => handleDelete(row)}>
                      삭제
                    </button>
                  </>
                )}
      ...
      <section className="contentGrid">
        <Panel title={isPermission ? '권한 설정' : '상세정보'} action={isPermission ? '권한 저장' : '상세 확인'} onAction={() => handleMock('실제 저장 없이 화면 상태만 변경되었습니다.')}>
          {isPermission ? (
