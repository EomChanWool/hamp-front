...
const dummyProductionResults: ProductionResultRow[] = [
  { resultNo: 'PR-F-3100', workOrderNo: 'WO-F-2400', itemName: '헴프 오일', producedQty: '820 kg', defectQty: '8 kg', yieldRate: '98%', registeredAt: '2026-06-22 09:20' },
  { resultNo: 'PR-F-3101', workOrderNo: 'WO-F-2401', itemName: '헴프 분말', producedQty: '916 kg', defectQty: '9 kg', yieldRate: '97.6%', registeredAt: '2026-06-21 10:20' },
  { resultNo: 'PR-F-3102', workOrderNo: 'WO-F-2402', itemName: '단백질 바', producedQty: '1012 kg', defectQty: '10 kg', yieldRate: '97.2%', registeredAt: '2026-06-20 11:20' },
  { resultNo: 'PR-F-3103', workOrderNo: 'WO-F-2403', itemName: '헴프 음료', producedQty: '1108 kg', defectQty: '11 kg', yieldRate: '96.8%', registeredAt: '2026-06-19 12:20' },
  { resultNo: 'PR-F-3104', workOrderNo: 'WO-F-2404', itemName: '씨드 그래놀라', producedQty: '1204 kg', defectQty: '12 kg', yieldRate: '96.4%', registeredAt: '2026-06-18 13:20' },
  { resultNo: 'PR-F-3105', workOrderNo: 'WO-F-2405', itemName: '헴프 캡슐', producedQty: '1300 kg', defectQty: '13 kg', yieldRate: '96%', registeredAt: '2026-06-17 14:20' },
]
...
const productionResultChart = {
  title: '식품 실적 추이',
  items: [
    { label: '원료투입', value: 45, tone: 'info' as StatusTone },
    { label: '세척', value: 53, tone: 'info' as StatusTone },
    { label: '건조', value: 61, tone: 'info' as StatusTone },
    { label: '분쇄', value: 69, tone: 'info' as StatusTone },
    { label: '선별', value: 77, tone: 'info' as StatusTone },
    { label: '포장', value: 85, tone: 'info' as StatusTone },
  ],
}

const PAGE_SIZE = 10

export function FoodProductionResultsPage() {
  const [filteredResults, setFilteredResults] = useState<ProductionResultRow[]>(dummyProductionResults)
  const [modalResult, setModalResult] = useState<ProductionResultRow | null>(null)
  ...
  const handleSearch = () => {
    const registeredStart = registeredStartRef.current?.value ?? ''
    const registeredEnd = registeredEndRef.current?.value ?? ''
    ...
    setFilteredResults(
      dummyProductionResults.filter(
        (result) =>
          (!registeredStart || result.registeredAt.slice(0, 10) >= registeredStart) &&
          (!registeredEnd || result.registeredAt.slice(0, 10) <= registeredEnd) &&
  ...
  const handleDelete = (result: ProductionResultRow) => {
  ...
  const handleSave = (updated: Record<string, string>) => {
    setFilteredResults((prev) => prev.map((r) => (r === modalResult ? ({ ...r, ...updated } as ProductionResultRow) : r)))
    setModalResult(null)
  ...
  const columns: ColumnDef<ProductionResultRow>[] = useMemo(
    () => [
  ...
  return (
    <section className="screenStack">
      ...
      <Panel title="식품 생산실적관리 목록" action="새로고침" onAction={() => window.alert('mock 새로고침')}>
