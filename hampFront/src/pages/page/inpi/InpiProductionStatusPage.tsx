...
interface InpiProductionStatus {
  workOrderNo: string
  itemName: string
  process: string
  progressRate: string
...
const dummyProductionStatus: InpiProductionStatus[] = [
  { workOrderNo: 'WO-I-2500', itemName: '인피 원면', process: '투입', progressRate: '42%', producedQty: '520 kg', manager: '김민준', status: '대기', startedAt: '2026-06-22 07:20' },
  { workOrderNo: 'WO-I-2501', itemName: '인피 섬유', process: '개섬', progressRate: '51%', producedQty: '590 kg', manager: '이서연', status: '진행중', startedAt: '2026-06-21 08:20' },
  { workOrderNo: 'WO-I-2502', itemName: '인피 매트', process: '정렬', progressRate: '60%', producedQty: '660 kg', manager: '박지훈', status: '완료', startedAt: '2026-06-20 09:20' },
  { workOrderNo: 'WO-I-2503', itemName: '인피 패드', process: '압축', progressRate: '69%', producedQty: '730 kg', manager: '최유진', status: '대기', startedAt: '2026-06-19 10:20' },
  { workOrderNo: 'WO-I-2504', itemName: '인피 롤', process: '포장', progressRate: '78%', producedQty: '800 kg', manager: '정도윤', status: '진행중', startedAt: '2026-06-18 11:20' },
  { workOrderNo: 'WO-I-2505', itemName: '인피 보드', process: '투입', progressRate: '87%', producedQty: '870 kg', manager: '한수아', status: '완료', startedAt: '2026-06-17 12:20' },
]
...
const productionStatusChart = {
  title: '공정별 진행률',
  items: [
    { label: '투입', value: 36, tone: 'good' as StatusTone },
    { label: '개섬', value: 46, tone: 'warn' as StatusTone },
    { label: '정렬', value: 56, tone: 'good' as StatusTone },
    { label: '압축', value: 66, tone: 'good' as StatusTone },
    { label: '포장', value: 76, tone: 'good' as StatusTone },
  ],
}

const PAGE_SIZE = 10

export function InpiProductionStatusPage() {
  const [filteredStatus, setFilteredStatus] = useState<InpiProductionStatus[]>(dummyProductionStatus)
  const [modalStatus, setModalStatus] = useState<InpiProductionStatus | null>(null)
  ...
  const handleSearch = () => {
    ...
    setFilteredStatus(
      dummyProductionStatus.filter((status) => {
        const date = status.startedAt.slice(0, 10)
        return (
          (!start || date >= start) &&
          (!end || date <= end) &&
  ...
  const handleDelete = (status: InpiProductionStatus) => {
    if (window.confirm(`${status.workOrderNo} 작업현황을 삭제할까요?`)) {
  ...
  const handleSave = (updated: Record<string, string>) => {
    setFilteredStatus((prev) => prev.map((s) => (s === modalStatus ? ({ ...s, ...updated } as InpiProductionStatus) : s)))
    setModalStatus(null)
  ...
  const columns: ColumnDef<InpiProductionStatus>[] = useMemo(
    () => [
      ...
      { accessorKey: 'manager', header: '담당자' },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return <Badge tone={value === '완료' ? 'good' : value === '진행중' ? 'info' : 'warn'}>{value}</Badge>
        },
      },
  ...
  return (
    <section className="screenStack">
      ...
      <Panel title="인피 생산현황 목록" action="새로고침" onAction={() => window.alert('mock 새로고침')}>
