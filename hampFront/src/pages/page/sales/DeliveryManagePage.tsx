...
const dummyDeliveries: DeliveryRow[] = [
  { orderNo: 'ORD-2026001', client: '(주)거래처A', clientManager: '김영업', deliveryItem: '헴프 오일 출고분', quantity: '1000', deliveryLocation: '본사창고', manager: '박납품', status: '완료', note: '-' },
  { orderNo: 'ORD-2026002', client: '(주)거래처A', clientManager: '김영업', deliveryItem: '헴프 분말 출고분', quantity: '1000', deliveryLocation: '본사창고', manager: '박납품', status: '완료', note: '-' },
  { orderNo: 'ORD-2026003', client: '(주)거래처A', clientManager: '김영업', deliveryItem: '단백질 바 세트', quantity: '1000', deliveryLocation: '본사창고', manager: '박납품', status: '완료', note: '-' },
  { orderNo: 'ORD-2026004', client: '(주)거래처A', clientManager: '김영업', deliveryItem: '헴프 음료 박스', quantity: '1000', deliveryLocation: '본사창고', manager: '박납품', status: '완료', note: '-' },
  { orderNo: 'ORD-2026005', client: '(주)거래처A', clientManager: '김영업', deliveryItem: '씨드 그래놀라 벌크', quantity: '1000', deliveryLocation: '본사창고', manager: '박납품', status: '완료', note: '-' },
  { orderNo: 'ORD-2026006', client: '(주)거래처A', clientManager: '김영업', deliveryItem: '헴프 캡슐 완제품', quantity: '1000', deliveryLocation: '본사창고', manager: '박납품', status: '완료', note: '-' },
]

const PAGE_SIZE = 10

export function DeliveryManagePage() {
  const [filteredDeliveries, setFilteredDeliveries] = useState<DeliveryRow[]>(dummyDeliveries)
  const [modalDelivery, setModalDelivery] = useState<DeliveryRow | null>(null)
  ...
  const handleSearch = () => {
    ...
    // 현재는 더미납품에 필터로 걸러내고 있는데 추후에 api 연동할 것
    setFilteredDeliveries(
      dummyDeliveries.filter(
        (delivery) =>
          ...
          (!status || delivery.status.includes(status)) &&
          (!note || delivery.note.includes(note)),
      ),
    )
  }

  const handleReset = () => {
    ;[orderNoRef, clientRef, clientManagerRef, deliveryItemRef, quantityRef, deliveryLocationRef, managerRef, statusRef, noteRef].forEach((ref) => {
      if (ref.current) ref.current.value = ''
    })
    setFilteredDeliveries(dummyDeliveries)
  }

  const handleDelete = (delivery: DeliveryRow) => {
    if (window.confirm(`${delivery.orderNo} 납품을 삭제할까요?`)) {
      setFilteredDeliveries((prev) => prev.filter((d) => d !== delivery))
      window.alert('mock data에서만 삭제되었습니다.')
    }
  }

  const handleSave = (updated: Record<string, string>) => {
    setFilteredDeliveries((prev) => prev.map((d) => (d === modalDelivery ? ({ ...d, ...updated } as DeliveryRow) : d)))
  ...
  const pagedDeliveries = filteredDeliveries.slice(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE)
  ...
  return (
    <section className="screenStack">
      ...
      <Panel title="납품관리 목록" action="등록" onAction={() => window.alert('등록 기능은 API 연동 후 사용 가능합니다.')}>
        <CusTable data={pagedDeliveries} columns={columns} onRowClick={setModalDelivery} />
        <CusPagination page={page} totalPages={totalPages} totalCount={filteredDeliveries.length} onPageChange={setPage} />
      </Panel>
