...
export const mesScreens = {
  ...
  orderManage: list(
  '수주관리',
  ['거래처', '수주번호', '거래처담당자', '생산품목', '생산량', '수주금액', '납기일', '상태', '담당자', '비고'],
  ['거래처', '수주번호', '거래처담당자', '생산품목', '생산량', '수주금액', '납기일', '상태', '담당자', '비고'],
  orderItems.map((item, i) => {
    // 🔴 다양성을 주기 위한 임시 배열들
    const vendors = ['(주)거래처A', '(주)대한상사', '(주)한국테크', '(주)미래물산'];
    const dates = ['2026-01-15', '2026-02-20', '2026-03-10', '2026-04-05', '2026-05-12', '2026-06-25'];
    
    return [
      vendors[i % vendors.length],                   // 거래처가 돌아가면서 들어가도록 수정
      `ORD-${2026001 + i}`,
      '김영업',
      item,                                          // 생산품목
      String(1000 + (i * 150)),                      // 생산량 변화 주기
      String(5000000 + (i * 500000)),                // 수주금액 변화 주기
      dates[i % dates.length],                       // 🔴 납기일(날짜)이 월별로 분산되도록 수정
      i % 2 === 0 ? '진행중' : '완료',
      '이수주',
      '-'
    ];
